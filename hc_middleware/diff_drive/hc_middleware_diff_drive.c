 ...
 * - motor driver 运行态访问接口：Motor_Get*/Motor_Set*
 *
 * 注意：
 ...
 * - 通过 Motor_Set*/Motor_Get* 读写运行态：encoder_total、encoder_delta、speed
...
static void encoder_update_one_wheel(Encoder_WheelId_e wheel, HC_S32 total_count)
{
    ...
    Motor_Id_e motor_id = cfg->motor_id;
    int32_t prev_total = 0;

    (void)Motor_GetEncoderTotal(motor_id, &prev_total);
    HC_S32 delta = total_count - (HC_S32)prev_total;

    (void)Motor_SetEncoderTotal(motor_id, (int32_t)total_count);
    (void)Motor_SetEncoderDelta(motor_id, (int16_t)delta);
    (void)Motor_SetSpeed(motor_id, Encoder_CalcSpeed((int32_t)delta) * (float)cfg->direction_sign);
}
...
void Encoder_Init(void)
{
    ...
    if (encoder_read_totals(totals) != 0) {
        (void)Motor_SetEncoderTotal(MOTOR_LEFT,  (int32_t)totals[ENCODER_WHEEL_LEFT]);
        (void)Motor_SetEncoderTotal(MOTOR_RIGHT, (int32_t)totals[ENCODER_WHEEL_RIGHT]);
    }
    else {
        (void)Motor_SetEncoderTotal(MOTOR_LEFT,  0);
        (void)Motor_SetEncoderTotal(MOTOR_RIGHT, 0);
    }

    /* 清空运行时缓存，保证 PID 启动前状态干净。 */
    for (int i = 0; i < MOTOR_COUNT; ++i) {
        (void)Motor_SetEncoderDelta((Motor_Id_e)i, 0);
        (void)Motor_SetSpeed((Motor_Id_e)i, 0.0f);
    }
}
...
void Encoder_GetTotals(int32_t* p_left_total, int32_t* p_right_total)
{
    if (p_left_total != NULL) {
        (void)Motor_GetEncoderTotal(MOTOR_LEFT, p_left_total);
    }
    if (p_right_total != NULL) {
        (void)Motor_GetEncoderTotal(MOTOR_RIGHT, p_right_total);
    }
}
